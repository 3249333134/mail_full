...
const JourneyEngine = {
  ...
  /** 从 CARRIER_ROSTER 中按 id 查找信使（兼容数组与 {byId} 对象两种结构），找不到返回 null */
  _findCarrier(carrierId) {
    const roster = window.CARRIER_ROSTER || CARRIER_ROSTER;
    if (!roster) return null;
    if (Array.isArray(roster)) {
      return roster.find((c) => c && c.id === carrierId) || null;
    }
    if (typeof roster.byId === 'function') return roster.byId(carrierId) || null;
    return null;
  },
  ...
  _letterCtx(letter) {
    if (!letter) return {};
    return { sender: letter.sender || '', recipient: letter.recipient || '', title: letter.letterTitle || '' };
  },

  /** 调用服务端 /api/ai/generate-journey，成功返回 { expectedDelivery, events, epilogue }，失败返回 null */
  async _aiGenerate(carrier, letter) {
    try {
      ...
      let res;
      try {
        res = await fetch(`${apiBase}/api/ai/generate-journey`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ carrier, letter: this._letterCtx(letter) }),
          signal: ctrl.signal,
        });
      } finally {
        clearTimeout(timer);
      ...
      if (!res || !res.ok) {
        console.warn('[JourneyEngine] /api/ai/generate-journey 返回 ' + (res && res.status || '无响应') + '，回退本地旅程模板');
        return null;
      }
      const data = await res.json();
      if (!data || typeof data !== 'object') return null;
      const events = Array.isArray(data.events)
        ? data.events.filter(e => e && typeof e.description === 'string').slice(0, 20)
        : [];
      if (!data.expectedDelivery && !data.epilogue && !events.length) return null;
      return {
        expectedDelivery: typeof data.expectedDelivery === 'string' ? data.expectedDelivery : '',
        events,
        epilogue: typeof data.epilogue === 'string' ? data.epilogue : '',
      };
    } catch (e) {
      console.warn('[JourneyEngine] AI 旅程生成异常，回退本地模板:',
        (e && e.name === 'AbortError') ? 'timeout(45s)' : String((e && e.message) || e));
      return null;
    }
  },

  /** 预期抵达：AI 优先（缓存），AI 失败返回 null（由调用方决定本地兜底展示） */
  _aiPending: {},
  isGenerating(carrierId) { return !!this._aiPending[carrierId]; },

  async aiExpected(carrier, letter) {
    if (!carrier) return this.fuzzyTime(carrier);
    if (this._aiCache[carrier.id]) return this._aiCache[carrier.id];
    if (this._aiPending[carrier.id]) return null;      // 已有请求在途，避免并发重复请求
    this._aiPending[carrier.id] = true;
    try {
      const ai = await this._aiGenerate(carrier, letter);
      if (ai && ai.expectedDelivery) {
        this._aiCache[carrier.id] = ai.expectedDelivery;
        return ai.expectedDelivery;
      }
      return null;    // AI 失败 → 返回 null，由调用方决定兜底展示
    } finally {
      delete this._aiPending[carrier.id];
    }
  },

  /** 用 AI 生成的事件链替换本地模拟链（结构兼容 plannedEvents） */
  _applyAiJourney(j, carrier, ai, mode, now) {
    if (ai.expectedDelivery) j.expectedDelivery = ai.expectedDelivery;
    if (ai.epilogue) j.aiEpilogue = ai.epilogue;
    const evs = (ai.events || []).filter(e => e && e.description);
    if (evs.length < 3) return;

    const species = this._speciesName(carrier.id);
    const planned = [];
    let t = 0;
    const pushEvt = (type, desc) => {
      t += Math.floor(this._rand(1, 6));
      planned.push({
        id: 'evt-ai-' + planned.length + '-' + Math.random().toString(36).slice(2, 6),
        type, time: t,
        location: { x: Math.floor(this._rand(0, 100)), y: Math.floor(this._rand(0, 100)) },
        actor: { name: carrier.name, species },
        description: desc,
        effects: { stateChange: {} },
      });
    };

    // 首事件：departure（AI 首个若是则 time=0 直接使用，否则补一条本地启程文案）
    const first = evs[0];
    if (first.type === 'departure') {
      planned.push({
        id: 'evt-ai-0-' + Math.random().toString(36).slice(2, 6),
        type: 'departure', time: 0,
        location: { x: 0, y: 0 },
        actor: { name: carrier.name, species },
        description: first.description,
        effects: { stateChange: {} },
      });
      for (const e of evs.slice(1)) pushEvt(e.type, e.description);
    } else {
      planned.push({
        id: 'evt-ai-0-' + Math.random().toString(36).slice(2, 6),
        type: 'departure', time: 0,
        location: { x: 0, y: 0 },
        actor: { name: carrier.name, species },
        description: `${carrier.name}衔起这封信，踏上了旅程。`,
        effects: { stateChange: {} },
      });
      for (const e of evs) pushEvt(e.type, e.description);
    }

    // 末事件：强制 delivery（AI 末条已是则保留，否则追加）
    const last = planned[planned.length - 1];
    if (last.type !== 'delivery') {
      pushEvt('delivery', '历经千山万水，信终于递到了收信人手上。');
    }

    j.plannedEvents = planned;
    j.totalTime = planned.length ? planned[planned.length - 1].time : 0;
    j.carrierChain = [
      { name: carrier.name, species, generation: 1, role: '启程' },
      { name: '收信人', species: '人', role: '收信' },
    ];

    if (mode === 'transit') {
      j.timeScale = j.deliverAt
        ? j.totalTime / Math.max(1, (j.deliverAt - now) / 1000)
        : j.totalTime / this._defaultSeconds(carrier);
      j.events = planned.slice(0, 1);
      j.eventCursor = 1;
      this._applyStateChange(j.letterState, planned[0] ? (planned[0].effects || {}).stateChange : null);
    } else {
      j.events = planned.slice();
      j.eventCursor = planned.length;
      j.status = 'delivered';
      j.deliverTime = now;
      j.report = this.buildReport(j, carrier);
    }
  },
  ...
  startJourney(letter, carrierId, opts = {}) {
    const roster = window.CARRIER_ROSTER || CARRIER_ROSTER;
    const carrier = this._findCarrier(carrierId) || (Array.isArray(roster) ? roster[0] : null);
    const now = Date.now();
  ...
  /**
   * 寄出：异步增强 —— AI 生成预期抵达 / 中途经历 / 结语并替换本地结果（失败静默回退本地）
   * 返回 Promise<journey>
   */
  async startJourneyWithAI(letter, carrierId, opts = {}) {
    const journey = this.startJourney(letter, carrierId, opts);
    const roster = window.CARRIER_ROSTER || CARRIER_ROSTER;
    const carrier = this._findCarrier(carrierId) || (Array.isArray(roster) ? roster[0] : null);
    const ai = await this._aiGenerate(carrier, letter);
    if (ai) this._applyAiJourney(journey, carrier, ai, journey.mode, Date.now());
    return journey;
  },
  ...
  tick(letter, now = Date.now()) {
    ...
    if (j.eventCursor >= j.plannedEvents.length) {
      j.status = 'delivered';
      j.deliverTime = now;
      const carrier = this._findCarrier(j.carrierId);
      j.report = this.buildReport(j, carrier);
  ...
  _simulateJourney(journey, initialCarrier, opts) {
    ...
    for (let i = 0; i < eventCount && transfers + lineages < 8; i++) {
      ...
      if (carrier.predationRate > 0 && this._hit(carrier.predationRate) && carrier.predators && carrier.predators.length) {
        ...
        // 切换载体
        const nextCarrier = this._findCarrier(predatorId) || carrier;
        carrier = nextCarrier;
        generation = 1;
  ...
  buildReport(journey, initialCarrier) {
    const roster = window.CARRIER_ROSTER || CARRIER_ROSTER;
    const carrier = this._findCarrier(journey.carrierId) || initialCarrier || (Array.isArray(roster) ? roster[0] : null);
    ...
    const report = {
      ...
      epilogue: (journey.aiEpilogue && journey.aiEpilogue.trim())
        ? journey.aiEpilogue
        : (chain.length > 2
