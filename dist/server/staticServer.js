...
// ---------- 后端 API 反向代理配置 ----------
// 前端脚本可能因时序/缓存回退到相对路径 /api/xxx，此处统一转发到真实后端（默认 3000）。
// 这样即使 MailService 未就绪，bootstrap / assets / ws 也一定能到达后端。
const BACKEND_HOST = process.env.BACKEND_HOST || '127.0.0.1';
const BACKEND_PORT = Number(process.env.BACKEND_PORT || 3000);

/** 判断路径是否需要转发到后端（API / 管理后台 / WebSocket 握手） */
function isBackendPath(pathname) {
  return pathname.startsWith('/api/') || pathname.startsWith('/admin') || pathname.startsWith('/ws');
}

/** HTTP 反向代理：透传 method / headers / body，透回状态码与字节流 */
function proxyHttp(req, res, pathname, url) {
  const headers = { ...req.headers };
  headers.host = `${BACKEND_HOST}:${BACKEND_PORT}`;
  // 删除 hop-by-hop 头（HTTP/1.1 语义）
  ['connection', 'proxy-connection', 'keep-alive', 'transfer-encoding', 'te', 'trailer', 'upgrade'].forEach(h => delete headers[h]);
  ...
  outbound.on('timeout', () => { outbound.destroy(new Error('proxy_timeout')); });
  req.pipe(outbound);
}

/** WebSocket 反向代理：双向字节透传（万物送信/多人模式依赖 WS） */
function proxyWs(req, socket, head, pathname, url) {
  const outbound = http.request({
    host: BACKEND_HOST,
    port: BACKEND_PORT,
    method: 'GET',
  ...
  outbound.on('upgrade', (upRes, upSocket, upHead) => {
  ...
  outbound.on('error', () => { try { socket.destroy(); } catch (_) {} });
  outbound.end();
}
...
const server = http.createServer((req, res) => {
  try {
    ...
    // 后端 API / 管理后台路径 → 反代到 3000 端口
    if (isBackendPath(pathname)) {
      proxyHttp(req, res, pathname, url);
      return;
    }
...
// ---------- 启动 ----------
// WebSocket 反向代理：前端 ws://host:3005 无缝连接后端 ws://host:3000
server.on('upgrade', (req, socket, head) => {
  try {
    const url = new URL(req.url, `http://${req.headers.host || 'localhost'}`);
    const pathname = decodeURIComponent(url.pathname);
    if (isBackendPath(pathname)) {
      proxyWs(req, socket, head, pathname, url);
      return;
    }
    socket.destroy();
  } catch (_) {
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`  API 反代: /api/** → ${BACKEND_HOST}:${BACKEND_PORT}（含 WebSocket）`);
  console.log(`  特性:     SPA fallback + Gzip + 长缓存 + API 反向代理`);
  console.log('══════════════════════════════════════════');
});
