...
/**
 * 迁移核心逻辑（供 CLI main 与服务启动 autoMigrateIfNeeded 复用）。
 * @param {object} args - 与 CLI 参数同构：{ include, dryRun, limit, batchSize, concurrency, resume }
 */
async function runMigration(args = {}) {
  ...
  // 断点续传：已成功的跳过
  let state = args.resume === false ? { done: {} } : readState();
  ...
  if (args.dryRun) {
    ...
    return { dryRun: true, total: fileList.length, pending: pending.length };
  }

  await mysqlClient.initMysql();
  if (!mysqlClient.isMysqlEnabled()) {
    console.error('MySQL 不可用，迁移中止');
    throw new Error('mysql_disabled');
  }
  ...
  return { ok: okCount, skipped: skipCount, failed: failCount, oversize: oversizeCount, total };
}

/** CLI 入口：解析 process.argv 后执行迁移 */
async function main() {
  const args = parseArgs(process.argv);
  const result = await runMigration(args);
  if (result && result.dryRun) process.exit(0);
}

/**
 * 服务启动自动迁移：asset_files 表为空（或 force=true）时执行全量迁移。
 * 幂等：非空表不重跑，避免每次启动扫描几千个文件。
 */
async function autoMigrateIfNeeded(options = {}) {
  await mysqlClient.initMysql();
  if (!mysqlClient.isMysqlEnabled()) {
    console.warn('[bootstrap] MySQL 不可用，跳过资产自动迁移（前端走本地静态兜底）');
    return { skipped: true, reason: 'mysql_disabled' };
  }
  const count = await mysqlDao.countAssets().catch(() => -1);
  if (!options.force && count > 0) {
    console.log(`[bootstrap] asset_files 已有 ${count} 条资产，跳过自动迁移`);
    return { skipped: true, existing: count };
  }
  console.log(`[bootstrap] asset_files 为空（count=${count}），开始自动迁移资产...`);
  ...
  console.log(`[bootstrap] 资产自动迁移完成: ${JSON.stringify(result)}`);
  return result;
}

module.exports = { main, runMigration, autoMigrateIfNeeded, DEFAULT_DIRS };
